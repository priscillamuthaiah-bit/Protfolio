...
contactForm.addEventListener('submit', (e) => {
    ...
    // Print logs (Scrubbed phone number)
    executeCommand(`ssh-send --payload-sender "${sender}" --subj "${subject}"`);
    
    setTimeout(() => {
        printTerminalLine(`[SSH-TRANSMIT] Connecting to secure proxy nodes...`, 'comment-line');
    }, 500);

    setTimeout(() => {
        printTerminalLine(`[SSH-TRANSMIT] Initializing 4096-bit RSA handshake with host priscillamuthaiah@gmail.com...`, 'comment-line');
    }, 1200);
    ...
    setTimeout(() => {
        ...
        alert(`Secure transmission packet from ${sender} has been successfully sent to Priscilla!`);
    }, 3000);
});
